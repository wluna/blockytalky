...
Blockly.Python.sensor_touch=function() {
    ...
    if (a == 1){
        return['(self.robot["sensors"]['+(b-1)+'] < 200) or (self.robot["sensors"]['+(b-1)+'] == 1',Blockly.Python.ORDER_ATOMIC];}
    if (a == 0){
        return['(self.robot["sensors"]['+(b-1)+'] > 200) or (self.robot["sensors"]['+(b-1)+'] == 0'
};
